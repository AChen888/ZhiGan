...
extern uint8_t oneflag;

static ui_window_t WinRecord =
{
	.pos.x = 0,
	.pos.y = 24,
	.pos.width = 320,
...
Local_BinTimeTypeDef TimeSelected;
static u16 ItemSelected = 0;
static u16 ItemTotal = 8;
...
static void Win_Init ( void )
{
	...
	WinRecord.sublist = 0;
	WinRecord.tick.step = 0;
	WinRecord.tick.period = 999;
	WinRecord.tick.lasttick = HAL_GetTick();

	snprintf(string, sizeof(string), "数据记录-%s", CONC_NAME);
...
static void Win_Paint ( void )
{
	...
	if(MenuSelected == 0)
	{
		...
		ui_show_lable(0, LINE_Y(0), LABLE_W, LINE_H, "通道" , efsize_24, rgb565_lable_text, rgb565_grid_line1, ( eat_hleft | eat_vcenter ));
		ui_show_lable(0, LINE_Y(1), LABLE_W, LINE_H, "年"   , efsize_24, rgb565_lable_text, rgb565_grid_line2, ( eat_hleft | eat_vcenter ));
		ui_show_lable(0, LINE_Y(2), LABLE_W, LINE_H, "月"   , efsize_24, rgb565_lable_text, rgb565_grid_line1, ( eat_hleft | eat_vcenter ));
		ui_show_lable(0, LINE_Y(3), LABLE_W, LINE_H, "日"   , efsize_24, rgb565_lable_text, rgb565_grid_line2, ( eat_hleft | eat_vcenter ));
		ui_show_lable(0, LINE_Y(4), LABLE_W, LINE_H, "时"   , efsize_24, rgb565_lable_text, rgb565_grid_line1, ( eat_hleft | eat_vcenter ));
		ui_show_lable(0, LINE_Y(5), LABLE_W, LINE_H, "分"       , efsize_24, rgb565_lable_text, rgb565_grid_line2, ( eat_hleft | eat_vcenter ));
		ui_show_lable(0, LINE_Y(6), LABLE_W, LINE_H, "秒" , efsize_24, rgb565_lable_text, rgb565_grid_line1, ( eat_hleft | eat_vcenter ));
	}
	else
	{
	}
...
//static void ShowLine2(u16 line, u8 drawback)
//{
//	char string[32];
...
//			break;
//		case 6:	//
//			snprintf(string, sizeof(string), "%02d", TimeSelected.Seconds);
//			ui_show_lable(160, LINE_Y(line), DATA_W, LINE_H, string, efsize_24, rgb565_digital_front, bcolor, ( eat_hleft | eat_vcenter ));
//			break;
//		
//		
...
static void ShowLine1(u16 line, u32 index)
{
	char string[32];
	char datastr[8];
	...
	index = (index % GAS_RECORD_TOTAL);				//使用分钟数计算记录序号

	if(gas_ReadRecordEx(index, record.Data.buffer))	
	{
		bf_TimeStamp2LocalTime(record.Record.MinuteStamp, &time);

		switch(record.Record.Alarm[ChannelSelected])
		{
			case evda_Class1:
				alarm = "一级";
				break;
		...
		value = gas_PPMConverTo (record.Record.Concentration[ChannelSelected], gCalidata[ChannelSelected].Unit);
		ui_format_string(datastr, sizeof ( datastr ), (u32*)&value, DataFormat, 0);
		snprintf(string, sizeof(string), "%02d%02d%02d %02d:%02d:%02d %s %s", time.Year, time.Month, time.Day, time.Hours, time.Minutes,time.Seconds, 
												datastr, alarm);
	}
	else
	{
	...
	ui_show_lable(0, LINE_Y(line), LABLE_W, LINE_H, string, efsize_24, rgb565_digital_front, bcolor, ( eat_hleft | eat_vcenter ));
}
//分割线------------------------------------------------------------------------------------------------------------------
//static void ShowLine1Onesecond(u16 line, u32 index)
//{
//	char string[32];
//	char datastr[8];
//	//s_GasDataRecord record;
...
//	if(gas_ReadRecordExOS(index, oneseconddata.Data.buffer))	
//	{
//		bf_TimeStamp2LocalTime(oneseconddata.Record.MinuteStamp, &time);
...
//				break;
//		}
//		value = gas_PPMConverTo (oneseconddata.Record.Concentration[ChannelSelected], gCalidata[ChannelSelected].Unit);
//		ui_format_string(datastr, sizeof ( datastr ), (u32*)&value, DataFormat, 0);
//		snprintf(string, sizeof(string), "%02d%02d%02d %02d:%02d:%02d %s %s %u", time.Year, time.Month, time.Day, time.Hours, time.Minutes,time.Seconds,
//												datastr, alarm, oneseconddata.Record.AD0);
//		SEGGER_RTT_printf(0,"%02d%02d%02d %02d:%02d:%02d %s %s %u\n", time.Year, time.Month, time.Day, time.Hours, time.Minutes,time.Seconds,
//												datastr, alarm, oneseconddata.Record.AD0);
//	}
//	else
//	{
...
//	{
////		// 240 - 24 -24 = 192 = 32 * 6 , 最多显示6行
////		ui_show_lable(16, LINE_Y(0), LABLE_W, LINE_H, "通道" , efsize_24, rgb565_lable_text, rgb565_grid_line1, ( eat_hleft | eat_vcenter ));
////		ui_show_lable(16, LINE_Y(1), LABLE_W, LINE_H, "年"   , efsize_24, rgb565_lable_text, rgb565_grid_line2, ( eat_hleft | eat_vcenter ));
////		ui_show_lable(16, LINE_Y(2), LABLE_W, LINE_H, "月"   , efsize_24, rgb565_lable_text, rgb565_grid_line1, ( eat_hleft | eat_vcenter ));
////		ui_show_lable(16, LINE_Y(3), LABLE_W, LINE_H, "日"   , efsize_24, rgb565_lable_text, rgb565_grid_line2, ( eat_hleft | eat_vcenter ));
////		ui_show_lable(16, LINE_Y(4), LABLE_W, LINE_H, "时"   , efsize_24, rgb565_lable_text, rgb565_grid_line1, ( eat_hleft | eat_vcenter ));
...
static void Win_ModifyPaint ( void )
{
	...
	else
	{
		...
		index = Compute_CurrentSecond(&time);		//计算分钟时间戳
		ShowLine1(0, index++);
		ShowLine1(1, index++);
		ShowLine1(2, index++);
		ShowLine1(3, index++);
				...
				if(MenuSelected == 0) {
						switch(ItemSelected)	{
							...
							case 5: 	//分
								...
								break;
							case 6: 	//秒
								g_Digital.udata = TimeSelected.Seconds;
								ui_edit_create_ex((ui_widgets_t*)win, &g_EditData, g_Digital, edf_uint_2, 160, LINE_Y(ItemSelected), DATA_W, LINE_H, efsize_24);
								break;
							case 7:		//保存
									...
									if(TimeSelected.Seconds > 59)
										TimeSelected.Seconds = 59;
		...
		if(g_Digital.udata != g_EditData.udata)
		{
			...
			switch(ItemSelected)
			{
				...
				case 5:		//
					...
					break;
				case 6:		//
					TimeSelected.Seconds = g_Digital.udata;
					if(TimeSelected.Seconds > 59)
						TimeSelected.Seconds = 59;
					break;
			}
...
//void printRecoed(void)				//20230301
//{
//	
//	Local_BinTimeTypeDef time01;
//	
...
//	
//	time01.Year    = GetDate.Year;
//	time01.Month   = GetDate.Month;
//	time01.Day    = GetDate.Date;
//	time01.Hours   = GetTime.Hours;
//	time01.Minutes = GetTime.Minutes;
//	time01.Seconds = GetTime.Seconds;
//	
//	
//	
//	
...
//	u32 index1;
//	
////	index1 = Compute_CurrentSecond(&time01);
//	
//		DataFormat = gCalidata[0].Format;
//		SEGGER_RTT_printf ( 0, "\r\n");
//		SEGGER_RTT_printf ( 0, "Pring record start!!\r\n");
//		for(index=0;index<GAS_RECORD_TOTAL;index++)
//			{
//		//	index = (index % GAS_RECORD_TOTAL);				//使用分钟数计算记录序号

//			if(gas_ReadRecordEx(index, oneseconddata.Data.buffer))	
//			{
//					bf_TimeStamp2LocalTime(oneseconddata.Record.MinuteStamp, &time01);
...
//					ui_format_string(datastr, sizeof ( datastr ), (u32*)&value, DataFormat, 0);
//					snprintf(string, sizeof(string), "%02d%02d%02d %02d:%02d:%02d %s %u %.2f %.2f %s", time01.Year, time01.Month, time01.Day, time01.Hours, time01.Minutes, time01.Seconds,
//												datastr, oneseconddata.Record.AD0, (float)oneseconddata.Record.T/100, (float)oneseconddata.Record.H/100, alarm);
//		
...
//		SEGGER_RTT_printf ( 0, "Pring record start!!\r\n");
		for(index=0;index<GAS_RECORD_TOTAL;index++)
			...
			if(gas_ReadRecordEx(index, record.Data.buffer))	
			{
					bf_TimeStamp2LocalTime(record.Record.MinuteStamp*60, &time);
					...
					value = gas_PPMConverTo (record.Record.Concentration[0], gCalidata[0].Unit);
					ui_format_string(datastr, sizeof ( datastr ), (u32*)&value, DataFormat, 0);
					snprintf(string, sizeof(string), "%02d%02d%02d %02d:%02d %s %s %.2f %.2f %u", time.Year, time.Month, time.Day, time.Hours, time.Minutes,  
												datastr, alarm, (float)record.Record.T/100, (float)record.Record.H/100, record.Record.AD0);
...
//void printRecoedNow(void)				//20230301
//{
...
//					value = gas_PPMConverTo (oneseconddata.Record.Concentration[0], gCalidata[0].Unit);
//					ui_format_string(datastr, sizeof ( datastr ), (u32*)&value, DataFormat, 0);
//					snprintf(string, sizeof(string), "%02d%02d%02d %02d:%02d:%02d %s %u %.2f %.2f %s", ntime.Year, ntime.Month, ntime.Day, ntime.Hours, ntime.Minutes, ntime.Seconds,
//												datastr, oneseconddata.Record.AD0, (float)oneseconddata.Record.T/100, (float)oneseconddata.Record.H/100, alarm);
//		
//					SEGGER_RTT_printf ( 0, "%s\r\n",string);
//					
...
void formatRecoed(void)				//20230301
{
	oneflag=1;
...
//		SEGGER_RTT_printf ( 0, "Format record start!!\r\n");
		for(index=0;index<GAS_RECORD_TOTAL;index++)
		{
			offset = index * GAS_RECORD_SIZE;												//使用分钟数计算地址偏移
			addr = NORFLASH_RECORD_DATA_ADDR + offset;

			if( (addr & 0x00FFF) == 0 )															//一个sector开始位置
					w25qxx_erase_sector(addr / NORFLASH_SECTOR_SIZE);		//擦除扇区
			MX_IWDG_Feed();
			
		}
//		SEGGER_RTT_printf ( 0, "Format record end!!\r\n");
		oneflag=0;
}
...
* 创建第一个窗口
*/
void WinRecord_Create ( void *parg )
{
	( void ) parg;

	ui_window_create ( ( ui_window_t * ) parg, &WinRecord, Win_Callback );
	ui_widgets_set_top_win ( ( ui_widgets_t * ) &WinRecord );		//很重要，一定要有这一句，第一个运行的窗口
}
/**********************
  **********************/
